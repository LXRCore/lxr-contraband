RegisterNetEvent('lxr-sellcontraband:server:sell', function(data)
    local src = source
    ...
    if Player.Functions.RemoveItem(data.name, data.amount, data.slot) then
        ...
        --TriggerEvent('lxr-witness:server:WitnessAlert', GetEntityCoords(GetPlayerPed(src)), 'Someone\'s Selling Contraband')
    end
end)